...
glm::vec3 getContactPoint(const Cube& cube1, const Cube& cube2, int collisionType, glm::vec3 mtv, float mtvMagnitude) {
    ...
    // 
    // USING SAT SEPERATING AXIS TYPE TO DETERMINE CONTACT POINT
    // ----------------------------------------------------------------
    //if (collisionType == 1) {
    ...
    //USING MINKOWSKI DIFFERENCE TO DETERMINE CONTACT POINT
    // ----------------------------------------------------------------
        // Get the vertices of the cubes
    std::vector<glm::vec3> vertices1 = cube1.GetVertices();
    std::vector<glm::vec3> vertices2 = cube2.GetVertices();

    // Calculate the Minkowski Difference
    std::vector<glm::vec3> minkowskiDifference;
    for (glm::vec3& vertex1 : vertices1) {
        for (glm::vec3& vertex2 : vertices2) {
            minkowskiDifference.push_back(vertex1 - vertex2);
        }
    }

    // The contact point is the point in the Minkowski Difference that is closest to the origin
    glm::vec3 contactPoint = minkowskiDifference[0];
    float minDistance = glm::length(minkowskiDifference[0]);
    for (int i = 1; i < minkowskiDifference.size(); i++) {
        float distance = glm::length(minkowskiDifference[i]);
        if (distance < minDistance) {
            contactPoint = minkowskiDifference[i];
            minDistance = distance;
        }
    }
